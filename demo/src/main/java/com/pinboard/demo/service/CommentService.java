...
public class CommentService {
    ...
    public Comment updateComment(Long commentId, String newContent, User requestingUser) {
        ...
        if (requestingUser.getId().equals(comment.getAuthor().getId())) {
            ...
            // Opcionalmente, podemos adicionar uma marca de editado
            // comment.setEdited(true); // se adicionar este campo ao modelo Comment
            
            Comment savedComment = commentRepository.save(comment);
            
            // Notifica os observadores sobre a edição do comentário
            notificationService.notifyObservers("Comentário editado no pin: " + comment.getPin().getTitle(), 
    ...
    public void deleteComment(Long commentId, User requestingUser) {
        ...
        if (requestingUser.getId().equals(comment.getAuthor().getId())) {
            ...
            notificationService.notifyObservers("Comentário removido do pin: " + comment.getPin().getTitle(), 
                    comment.getPin().getId());
        }
    }
    ...
    public boolean toggleLikeComment(Long commentId, User user) {
        ...
        if (wasLiked) {
            ...
            notificationService.notifyObservers(
                "Usuário removeu like de um comentário no pin: " + comment.getPin().getTitle(),
                comment.getPin().getId());
        } else {
            comment.addLike(user);
            notificationService.notifyObservers(
                "Usuário curtiu um comentário no pin: " + comment.getPin().getTitle(),
                comment.getPin().getId());
        }
